...
test('scarfs testing', async t => {
  ...
  await t
    //.expect(developerNameInput.value).eql('-Я ищу...', 'input is empty')
    .typeText(developerNameInput, 'шарф FABRETTI')
    .expect(developerNameInput.value).contains('шарф FABRETTI', 'input contains text "sumki"')
    .pressKey("enter");
...
test('shapka testing', async t => {
  const developerNameInput = Selector('body > div.wrapper > header > div > div.header__bottom > div.header__search-catalog.search-catalog.j-search-catalog > div.search-catalog__block > input', { timeout: 5000 });
  await t
    //.expect(developerNameInput.value).eql('-Я ищу...', 'input is empty')
    .typeText(developerNameInput, 'шапка FABRETTI')
    .expect(developerNameInput.value).contains('шапка FABRETTI', 'input contains text "sumki"')
    .pressKey("enter");
